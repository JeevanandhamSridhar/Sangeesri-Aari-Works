...
const JSON_PATH = path.join(process.cwd(), 'src/data/blog-storage.json')
const CLIENT_BLOG_DIR = path.join(process.cwd(), 'public/blog')
const ADMIN_BLOG_DIR = path.join(process.cwd(), '../admin/public/blog')
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ success: true, posts: filtered }, { headers: corsHeaders })
}
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'upload_file' && fileData && fileName) {
      if (!fs.existsSync(CLIENT_BLOG_DIR)) fs.mkdirSync(CLIENT_BLOG_DIR, { recursive: true })
      if (!fs.existsSync(ADMIN_BLOG_DIR)) {
        try { fs.mkdirSync(ADMIN_BLOG_DIR, { recursive: true }) } catch {}
      }
      ...
      fs.writeFileSync(path.join(CLIENT_BLOG_DIR, cleanName), buffer)
      if (fs.existsSync(ADMIN_BLOG_DIR)) {
        try { fs.writeFileSync(path.join(ADMIN_BLOG_DIR, cleanName), buffer) } catch {}
      }

      const relativeSrc = `/blog/${cleanName}`
      return NextResponse.json({ success: true, uploadedSrc: relativeSrc, posts: current }, { headers: corsHeaders })
    }
    ...
    if (action === 'add_post' && post) {
      ...
      return NextResponse.json({ success: true, posts: current }, { headers: corsHeaders })
    }
