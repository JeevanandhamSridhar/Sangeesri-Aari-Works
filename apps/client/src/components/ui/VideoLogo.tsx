...
import { Maximize2, X, Sparkles, Volume2, VolumeX } from 'lucide-react'
  ...
  return (
    <>
      <div className={`flex items-center gap-3.5 ${className}`}>
        ...
        {showText && (
          <div>
            <span className="font-playfair text-xl font-bold tracking-tight text-cream block leading-none">
              Sangee Sri
            </span>
            <span className="font-inter text-[10px] font-semibold tracking-[0.25em] text-gradient-gold uppercase block mt-1">
              Aari Works · Studio
            </span>
          </div>
        )}
      ...
      {/* Fullscreen HD Video Lightbox Modal */}
      <AnimatePresence>
        {modalOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            >
              ...
              <div className="flex items-center justify-between pb-4 border-b border-white/10">
                <div className="flex items-center gap-2 text-gold-400 font-inter text-xs font-semibold uppercase tracking-wider">
                  <Sparkles size={16} /> Sangee Sri Aari Works — Brand Video Showcase
                </div>
                <button
                  onClick={() => setModalOpen(false)}
              ...
              <div className="flex items-center justify-between font-inter text-xs text-cream/60 pt-2">
                <span>📍 Studio: Banavaram Road, Kaveripakkam</span>
                <span className="text-gold-400 font-semibold">✨ Premium Animated Logo</span>
              </div>
            </motion.div>
          </motion.div>
        )}
